...
# Sample Data Structures for Tables and Bookings
class Table:
...
class Booking:
    def __init__(self, booking_id, customer_name, time_slot):
...
def book_table(table_id):
    ...
    if table and not table.is_booked:
        if request.method == 'POST':
            customer_name = request.form['customer_name']
            time_slot = request.form['time_slot']
            booking = Booking(len(bookings) + 1, customer_name, time_slot)
